...
class DiagonalLinear(nn.Module):
    def __init__(self, in_feature, out_feature, r=8, dtype=None):
        super(DiagonalLinear, self).__init__()
        self.r = r
        self.weight = nn.Parameter(torch.zeros(self.r, in_feature, dtype=dtype))
        self.weight_col = nn.Parameter(torch.zeros(1, out_feature, dtype=dtype))

    def forward(self, matrix, x):
        y_list = []
        for r_idx in range(self.r):
            y_list.append(F.linear(x * self.weight[r_idx], matrix[r_idx * matrix.shape[0]//self.r: (r_idx + 1) * matrix.shape[0]//self.r,:]))
    
        return torch.concat(y_list, dim=2) * self.weight_col
...
class Linear(nn.Linear, LoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if r > 0:
            self.lora_A = DiagonalLinear(in_features, out_features, r=16, dtype=self.weight.dtype)
    ...
    def train(self, mode: bool = True):
        ...
        if not mode and self.merge_weights and not self.merged:
            ...
            r1 = 16
            if self.r > 0:
                self.weight.data += torch.repeat_interleave(self.lora_A.weight, self.weight.shape[0]//r1, dim=0) * self.lora_A.weight_col.view(-1, 1) * self.weight.data * self.scaling
            self.merged = True
